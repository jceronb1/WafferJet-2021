...
public class Activity_RutaViaje_Maps extends FragmentActivity implements OnMapReadyCallback {
    ...
    private void setRoute(String key){
        Log.d("USPRUEBA", key +"1");
        mRef = mDatabase.getReference(PATH_ROUTES).child(key);
        mRef.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot snapshot) {
                Log.d("USPRUEBA", String.valueOf(snapshot.child("ruta").getChildrenCount()));

                //List<LatLng> route = snapshot.child("ruta").getValue(List.class);
                ...
                /*    PolylineOptions line = new PolylineOptions().width(8).color(mapFragment.getActivity().getResources().getColor(R.color.lightblue));
                    line.addAll(route);
